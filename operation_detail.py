...
def _url_is_stories_endpoint(u: str) -> bool:
    ul = (u or "").lower()
    return "api-stories" in ul and "/ib/api/stories/" in ul


def _manual_from_cached_operation(op_id: str) -> Optional[dict]:
    """Fallback: если нет manual/fake_history, берём поля из history.operations_cache."""
    try:
        op = (history.operations_cache or {}).get(op_id)
        if not isinstance(op, dict):
            return None
        typ = str(op.get("type") or "Debit").strip() or "Debit"
        title = str(op.get("title") or op.get("description") or "").strip() or "Перевод"
        return {
            "id": str(op_id or "").strip(),
            "type": typ,
            "amount": abs(float(op.get("amount") or 0)),
            "title": title,
            "subtitle": str(op.get("subtitle") or "").strip(),
            "description": str(op.get("description") or "").strip() or title,
            "phone": str(op.get("phone") or op.get("requisite_phone") or "").strip(),
            "requisite_phone": str(op.get("requisite_phone") or op.get("phone") or "").strip(),
            "requisite_sender_name": str(
                op.get("requisite_sender_name") or op.get("sender_name") or title
            ).strip(),
            "sender_name": str(op.get("sender_name") or "").strip(),
            "bank": str(op.get("bank") or "").strip(),
            "card_number": str(op.get("card_number") or "").strip(),
...
def request(flow: http.HTTPFlow) -> None:
    ...
    if _url_is_stories_endpoint(_url0):
        return
...
def response(flow: http.HTTPFlow) -> None:
    ...
    if _url_is_stories_endpoint(flow.request.pretty_url or ""):
        return
        ...
        for mid in ids_in_flow:
            ...
            if history.op_id_in_fake_history_files(mid):
                ...
                if hop:
                    target_manual = history.fake_history_record_as_manual_dict(hop)
                    break
            from_cache = _manual_from_cached_operation(mid)
            if from_cache:
                target_manual = from_cache
                break
