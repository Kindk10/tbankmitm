...
def neutralize_cx_aml_task(url: str, data) -> bool:
    """
    /cx-evolution-api/aml/task отвечает коротким JSON и может включать флаг,
    запускающий красную плашку «Временно заблокировали карту…».
    """
    u = (url or "").lower()
    if "/cx-evolution-api/aml/task" not in u:
        return False
    changed = False
    ...
    for k in ("task", "tasks", "items", "offers", "warnings", "alerts", "payload", "data"):
        if k in data:
            v = data.get(k)
    ...
    for k in (
        ...
        if data.get(k) not in (False, 0, "", None):
            data[k] = False
            changed = True
    ...
    try:
        ...
        if not ctx_stmt:
            ...
            if neutralize_stories_offers_aml(url, data):
                security_patched = True
            if neutralize_cx_aml_task(url, data):
                security_patched = True
